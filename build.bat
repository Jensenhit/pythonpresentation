...
set "option-bits=c:\sunano-bits"
set "option-build=%TEMP%\sunano-build"
set "option-bin=%TEMP%\sunano-bin"
...
if defined option-h (
  echo build.bat - sunano build script
  echo.
...
rem ----------------------------------------------------------
rem read sunano Version

for /F "tokens=1" %%i in ('type version.sh') do (
  set line=%%i
  if "!line:~0,16!" equ "sunano_VERSION=" (
    set "version=!line:~16,100!"
    set "sunano_VERSION=!version:"=!"
  )
)

date /t >%TEMP%\sunano-build-tmp.txt
set /P sunano_VERSION_DATE=<%TEMP%\sunano-build-tmp.txt
del %TEMP%\sunano-build-tmp.txt

rem The short SHA hash of the commit
git rev-parse --short HEAD 2>nul >%TEMP%\sunano-build-tmp.txt
set /P sunano_VERSION_REV=<%TEMP%\sunano-build-tmp.txt
if ERRORLEVEL 1 (
	set "sunano_VERSION_REV=LatestSourcePackage"
)
del %TEMP%\sunano-build-tmp.txt
...
echo.
echo CONFIG:                   %CONFIG%
echo sunano_VERSION:          %sunano_VERSION%
echo sunano_VERSION_DATE:     %sunano_VERSION_DATE%
echo sunano_VERSION_REV:      %sunano_VERSION_REV%
...
"%option-qmake%" ^
  ...
  -spec win32-msvc ^
  "CONFIG+=%CONFIG%" ^
  "sunano_VERSION=%sunano_VERSION%" ^
  "sunano_VERSION_DATE=%sunano_VERSION_DATE%" ^
  "sunano_VERSION_REV=%sunano_VERSION_REV%" ^
  ...
  %inst_path%\src\sunano.pro || exit /b 1 
